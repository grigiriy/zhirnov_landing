    ...
    <div className="relative w-full group">
      <label 
        className={`absolute left-0 transition-all duration-300 pointer-events-none font-['Inter'] font-medium tracking-tight ${
          focused || value 
            ? "-top-6 text-[13px] text-zinc-500 leading-tight opacity-100" 
            : "top-0 text-white text-2xl leading-8 opacity-50"
        }`}
      >
        {label}{required ? "*" : ""}
      </label>
...
const ContactForm: React.FC = () => {
  ...
  return (
    <section id="contacts" className="py-12 md:py-24 px-4 md:px-10 max-w-[1920px] mx-auto">
      <div className="bg-[#1a1a1a] rounded-[3rem] md:rounded-[4rem] p-5 md:py-[48px] md:px-[56px] text-white overflow-hidden relative">
        
        <h2 className="text-[48px] md:text-[86px] font-medium tracking-[-0.05em] leading-none mb-6 md:mb-[52px] whitespace-nowrap">
          Обсудить проект
        </h2>
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-[2.5rem] p-4 flex flex-col md:flex-row md:items-center justify-between gap-6 mb-3 md:mb-[52px] border border-white/5">
          ...
          <button className="h-[52px] w-full md:w-auto px-4 py-3.5 bg-white rounded-2xl inline-flex justify-center items-center gap-2 overflow-hidden hover:bg-zinc-100 transition-all active:scale-95 group">
            <span className="justify-center text-neutral-800 text-lg font-normal font-['Golos_Text'] leading-6">Написать в Telegram</span>
          </button>
        </div>

        {/* Form Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[26px] md:gap-10 mb-12 md:mb-[52px] items-end">
        ...
        {/* Submit Block */}
        <div className="mt-6 md:mt-[52px] p-4 bg-white/5 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-8">
           {/* White Button: 52px height, px-4 py-3.5, fullwidth mobile */}
